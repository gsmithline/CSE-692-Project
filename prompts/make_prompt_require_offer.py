...
def make_prompt(T: int, quantities: list[int], V: int, values: list[float], W1: int, W2: int, w: int, R: int, g: float, r: int, history: dict, current_offer: Offer = None, player_num: int = 0, p1_outside_offer: list[int] = None, p2_outside_offer: list[int] = None) -> str:
    ...
    2) Let's first come up with a potential counteroffer:
    - What would be a fair division of items?
    - Calculate the value of this division to me:
      For example, if I offer the other player 2 units of each item (keeping 3 units of each for myself):
      3 units of item 1 = 3 × {values[0]} = {3*values[0]} (multiplying units by my value per unit)
      3 units of item 2 = 3 × {values[1]} = {3*values[1]} (multiplying units by my value per unit)
      3 units of item 3 = 3 × {values[2]} = {3*values[2]} (multiplying units by my value per unit)
      3 units of item 4 = 3 × {values[3]} = {3*values[3]} (multiplying units by my value per unit)
      3 units of item 5 = 3 × {values[4]} = {3*values[4]} (multiplying units by my value per unit)
      Total value = {sum([3*values[i] for i in range(T)])} (sum of all item values)
    - Write down your proposed counteroffer and its value to you

    3) Now let's evaluate all options:
    - What is the current discount factor, gamma^(r-1)?
    - For the current offer (if any): What would be my total value if I accept?
    - For my potential counteroffer: What would be my total value?
    - For walking away: I would get my outside offer value
    - How does the discount factor affect these values?

    4) Let's decide between:
    - Accepting the current offer (if any)
    - Making my calculated counteroffer
    - Walking away with my outside offer
