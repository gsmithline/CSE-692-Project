...
for i in range(10000):
    item_counts = np.random.poisson(4, 5)
    p1_vals = np.random.randint(1, 101, 5)
    p2_vals = np.random.randint(1, 101, 5)

    result, alloc = compute_max_nash_welfare(item_counts, p1_vals, p2_vals)
    results = np.append(results, result)

global_max_nash_welfare = np.mean(results)
print("Max Nash Welfare:", global_max_nash_welfare)
